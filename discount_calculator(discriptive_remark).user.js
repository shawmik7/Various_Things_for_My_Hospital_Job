...
(function () {
    ...
    function applyDiscount(selectedPercent) {
        ...
        let normalDiscounts = new Set();
        let imagingDiscounts = new Set();
        ...
        testFields.forEach(testInput => {
            ...
            if (allowedPercent === 0) {
                ...
                if (isImaging) {
                    imagingDiscounts.add(allowedPercent);
                } else {
                    normalDiscounts.add(allowedPercent);
                }
            }
        ...
        // Round discount to make payable rounded to 10
        ...
        // Generate remark field text
        const allPercents = [...normalDiscounts, ...imagingDiscounts];
        const isCorporate = allPercents.some(p => p >= 50);
        let remarkText = isCorporate ? 'CORPORATE DISCOUNT ' : 'DISCOUNT BY DR ';

        // Check discount equality
        const normSet = [...normalDiscounts].sort((a,b)=>a-b);
        const imgSet = [...imagingDiscounts].sort((a,b)=>a-b);

        let sameDiscount = false;
        if (normSet.length === 1 && imgSet.length === 1 && normSet[0] === imgSet[0]) {
            sameDiscount = true;
        } else if (imgSet.length === 0 && normSet.length === 1) {
            sameDiscount = true;
        } else if (normSet.length === 0 && imgSet.length === 1) {
            sameDiscount = true;
        }

        if (sameDiscount) {
            remarkText += `(${normSet.length ? normSet[0] : imgSet[0]}%)`;
        } else {
            const percentSegments = [];

            if (normSet.length > 0) {
            const normalText = normSet.map(p => `${p}%`).join(', ');
            percentSegments.push(normalText);
            }

            if (imgSet.length > 0) {
                const imagingText = imgSet.map(p => `${p}%`).join(', ');
                percentSegments.push(`IMAGING ${imagingText}`);
            }

            remarkText += `(${percentSegments.join(', ')})`;
        }
        ...
        if (remarkField) {
            remarkField.value = remarkText.trim();
        }
    }
